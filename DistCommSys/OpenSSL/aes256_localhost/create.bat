...
openssl req -passin pass:1111 -new -x509 -days 3650 -key ca.key -out ca.crt -subj "/C=CN/ST=HuNan/L=ChangSha/O=CVICSE/OU=gRPC/CN=localhost"

openssl genrsa -passout pass:1111 -aes256 -out server_privatekey.pem 2048

openssl req -passin pass:1111 -new -key server_privatekey.pem -out server_csr.pem -subj "/C=CN/ST=HuNan/L=ChangSha/O=CVICSE/OU=gRPC/CN=localhost"
...
openssl req -passin pass:1111 -new -key client_privatekey.pem -out client_csr.pem -subj "/C=CN/ST=HuNan/L=ChangSha/O=CVICSE/OU=gRPC/CN=localhost"
